...
public class DistributedApplicationTests
{
    ...
    public async Task TestServicesWithMultipleReplicas()
    {
        ...
        try
        {
            var uri = app.GetEndpoint(testProgram.ServiceBBuilder, "http");
            while (true)
            {
    ...
    public async Task SpecifyingEnvPortInEndpointFlowsToEnv()
    {
        ...
        Assert.Equal("5031", GetEnv(nodeApp.Spec.Env, "PORT"));
    ...
    public async Task ReplicasAndProxylessEndpointThrows()
    {
        ...
        var suffix = app.Services.GetRequiredService<IOptions<DcpOptions>>().Value.ResourceNameSuffix;
        Assert.Equal($"'servicea_{suffix}' specifies multiple replicas and at least one proxyless endpoint. These features do not work together.", ex.Message);
    }
    ...
    {
        var builder = DistributedApplicationTestingBuilder.Create((appOptions, hostOptions) => appOptions.AssemblyName = typeof(DistributedApplicationTests).Assembly.FullName);
        ...
        using var clientA = app.CreateHttpClient(servicea, "http");
