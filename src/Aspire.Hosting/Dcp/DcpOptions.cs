...
internal class ConfigureDefaultDcpOptions(
    ...
    DistributedApplicationExecutionContext executionContext) : IConfigureOptions<DcpOptions>
{
    ...
    public void Configure(DcpOptions options)
    {
        ...
        {
            options.ContainerRuntime = configuration.GetValue<string>("DOTNET_ASPIRE_CONTAINER_RUNTIME");
        }
        ...
        {
            options.DependencyCheckTimeout = configuration.GetValue<int>("DOTNET_ASPIRE_DEPENDENCY_CHECK_TIMEOUT", options.DependencyCheckTimeout);
        }

        options.KubernetesConfigReadRetryCount = dcpPublisherConfiguration.GetValue<int>(nameof(options.KubernetesConfigReadRetryCount), options.KubernetesConfigReadRetryCount);
        options.KubernetesConfigReadRetryIntervalMilliseconds = dcpPublisherConfiguration.GetValue<int>(nameof(options.KubernetesConfigReadRetryIntervalMilliseconds), options.KubernetesConfigReadRetryIntervalMilliseconds);
        ...
        options.DeleteResourcesOnShutdown = dcpPublisherConfiguration.GetValue<bool>(nameof(options.DeleteResourcesOnShutdown), options.DeleteResourcesOnShutdown);
        options.RandomizePorts = dcpPublisherConfiguration.GetValue<bool>(nameof(options.RandomizePorts), options.RandomizePorts);
        options.ServiceStartupWatchTimeout = configuration.GetValue<TimeSpan>("DOTNET_ASPIRE_SERVICE_STARTUP_WATCH_TIMEOUT", options.ServiceStartupWatchTimeout);
    }
